...
if __name__=='__main__':
    ...
    if config.model=='resnet50':
        ...
        teacher.fc=torch.nn.Linear(in_feature,10)
        student.fc=torch.nn.Linear(in_feature,10)
    ...
    if config.dataset=='imagenet':
        ...
        mnist_dataset = MultilabelMNIST(config.mnist_path,config.model)
        train_dataloader = mnist_dataset.getDataloader(train=True)
    print('loading val dataset...')
    if config.dataset=='imagenet':
        imagenet_val_data = ImageNetReal(config.val_path,config.real_path)
        val_dataloader = imagenet_val_data.getDataloader(config.model)
    elif config.dataset=='mnist':
        val_dataloader = mnist_dataset.getDataloader(train=False)
